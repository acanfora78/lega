...
//
// Sezione volutamente separata dal Tabellino (gol, cartellini, voti): la
// distinta serve solo a dire chi c'era e chi partiva titolare.
// ============================================================================
      ...
      <CardContent className="flex flex-col gap-5 p-5">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <ColonnaDistinta
            ...
            vociIniziali={distintaTrasferta}
            indisponibili={indisponibili}
          />
        </div>
      </CardContent>
...
function ColonnaDistinta({
  ...
  function escludi(giocatoreId: string) {
    setVoci((v) => v.filter((voce) => voce.giocatoreId !== giocatoreId));
  }

  function aggiorna(giocatoreId: string, patch: Partial<FormazioneVoce>) {
    setVoci((v) => v.map((voce) => (voce.giocatoreId === giocatoreId ? { ...voce, ...patch } : voce)));
  }
      ...
      {roster.length === 0 ? (
        ...
        <>
          <div className="flex flex-wrap gap-2 py-1">
            ...
            <Button variant="outline" size="sm" onClick={() => setVoci([])} disabled={salvataggio || voci.length === 0}>
